...
import type { RateLimitsSnapshot } from "../../lib/rateLimits";
import {
...
const ComposerFooterPrimaryActions = memo(function ComposerFooterPrimaryActions(props: {
  ...
  activeRateLimits: RateLimitsSnapshot | null;
  ...
  return (
    <>
      {props.showSecondaryStatus && props.activeRateLimits ? (
        <RateLimitMeter limits={props.activeRateLimits} />
      ) : null}
...
export interface ChatComposerProps {
  ...
  // Context window
  activeContextWindow: ContextWindowSnapshot | null;
  activeRateLimitsSnapshot: RateLimitsSnapshot | null;
  compactDisabled: boolean;
  compactDisabledReason: string | null;
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  // Context window
  // ------------------------------------------------------------------
  // Account rate limits: Codex and Claude only, and only while the thread is
  // still on the provider that reported them.
  const activeRateLimits = useMemo(() => {
    const limits = activeRateLimitsSnapshot;
    return limits &&
      (selectedProvider === "codex" || selectedProvider === "claudeAgent") &&
      limits.provider === selectedProvider
      ? limits
      : null;
  }, [activeRateLimitsSnapshot, selectedProvider]);
  ...
  return (
    <form
      ...
      className="mx-auto w-full min-w-0 max-w-[var(--chat-max-width,48rem)]"
