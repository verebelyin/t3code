...
export function deriveMessagesTimelineRows(input: {
  ...
  // Expand-all mode renders every tool call as its own row, so the live
  // group never forms and the entries flow through the normal work path.
  const activeToolScanStart = input.expandAllToolCalls ? -1 : input.timelineEntries.length - 1;
  for (let index = activeToolScanStart; index >= activeTurnHeaderIndex; index -= 1) {
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    if (timelineEntry.kind === "work") {
      ...
      // what the mode exists to undo. Superseded lifecycle markers still
      // fold away so a call never renders twice.
      const visibleGroupedEntries = omitSupersededLifecycleMarkers(
        input.expandAllToolCalls
          ? groupedEntries
