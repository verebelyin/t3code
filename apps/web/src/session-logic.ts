...
function toolLifecycleCollapseMapKey(entry: DerivedWorkLogEntry): string | undefined {
  ...
  // The derived collapse key covers the toolCallId form and falls back to
  // label/detail identity for providers that never stamp call ids, so a
  // completion still finds its streaming row across interleaved entries.
